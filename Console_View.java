...
public class Console_View {
    ...
    public void Buy_view(Menu buy_burger, Order orders){
        ...
        System.out.println("1. " + opt_single + "(" + single_price + ")\t" + "2. " + opt_double + "(" + double_price + ")\n");
        int buy_check_opt = sc.nextInt();
        sc.nextLine();
        buy_burger.setOption((buy_check_opt == 1) ? opt_single : opt_double);
        buy_burger.setPrice((buy_check_opt == 1) ? single_price : double_price);
        System.out.println("\n");

        System.out.println(buy_burger.getName()+ "(" + buy_burger.getOption() + ")" + "\t|\t W" + buy_burger.getPrice() + "\t|\t" + buy_burger.getContent());
    ...
    public void Order_view(Order orders){
        while(true) {
            ...
            for (int i = 0; i < orders.size(); i++) {
                Burger burger_temp = (Burger) orders.getOrder_menu().get(i);
                System.out.println((burger_temp.getName() + "\t|\t" + burger_temp.getPrice() + "\t|" + burger_temp.getCount() + "개\t|t" + burger_temp.getContent()));
                total += burger_temp.getPrice() * (burger_temp.getCount());
            }
